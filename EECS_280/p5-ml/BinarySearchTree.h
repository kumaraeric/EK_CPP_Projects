...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    Node *node_copy = new Node(node->datum,node->left,node->right);
...
//Node(const T &datum_in, Node *left_in, Node *right_in)
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    Node* node_right_copy = node->right;
    Node* node_left_copy = node->left;
    delete node;
    node = nullptr;
    destroy_nodes_impl(node_right_copy);
    destroy_nodes_impl(node_left_copy);
  }
 ...
 // done
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(!empty_impl(node->left) && !less(max_element_impl(node->left)->datum,node->datum)){
      return false;
    }

    if(!empty_impl(node->right) && (less(min_element_impl
    (node->right)->datum,node->datum) || 
    (!less(min_element_impl(node->right)->datum,node->datum)&&
    !less(node->datum,min_element_impl(node->right)->datum)))){
      return false;
    }

    return true && check_sorting_invariant_impl(node->left,less)
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    static Node *min_max = nullptr;
    assert(check_sorting_invariant_impl(node,less));

    if(empty_impl(node) || (less(max_element_impl(node)->datum,val) 
    || (!less(max_element_impl(node)->datum,val) && 
    !less(val,max_element_impl(node)->datum)))){
      return nullptr;
    }
    //value < datum move left keep track of min_max value 
    //value >= datum move right
