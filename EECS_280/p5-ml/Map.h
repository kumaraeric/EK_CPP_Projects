...
private:
  ...
  class PairComp {
    public:
      bool operator()(Pair_type obj_1, Pair_type obj_2){
        Key_compare less;
        if(less(obj_1.first,obj_2.first)){
          return true; 
        }
        return false; 
      }
...
public:
  ...
  //copy constructor
  //done
  Map(const Map <Key_type,Value_type,Key_compare> &other):entries(other.entries){}

  //equal operator
  //done
  Map<Key_type,Value_type,Key_compare> operator=
  (const Map <Key_type,Value_type,Key_compare> &other){
  ...
  Value_type& operator[](const Key_type& k){
    pair<Iterator,bool> holder = insert({k,Value_type()});
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    Iterator holder = find(val.first);
    if(holder!=end()){
      return pair<Iterator,bool>{holder,false}; 
    }
    holder = entries.insert(val); 
    return pair<Iterator,bool>{holder,true};
  }
